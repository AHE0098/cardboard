...
function mountLegacyBattleInApp() {
  ...
  const SIM_USE_SNIPPETS = true;

  function renderSimulatorMode(rootNode) {
    if (!SIM_USE_SNIPPETS || !window.SimUI?.mountSimulator) {
      throw new Error("Simulator snippets not loaded");
    }
    window.SimUI.mountSimulator({
      containerEl: rootNode,
      apiBaseUrl: "",
      initialDeckId: simulatorState.deckAId || "",
      devFlags: {},
      context: {
        subtitle,
        session,
        ...
        parseManaCost,
        getCardDef,
        getAbortController: () => simulatorAbortController,
        setAbortController: (next) => { simulatorAbortController = next; }
      }
    });
  }
