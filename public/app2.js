...
function mountLegacyBattleInApp() {
  ...
  function renderSimulatorMode(rootNode) {
    ...
    if (simulatorState.summary) {
      ...
      const winrateFn = typeof buildWinrateCheckpoints === "function"
        ? buildWinrateCheckpoints
        : ((runsMeta = [], preferredN) => {
          if (!Array.isArray(runsMeta) || !runsMeta.length) return [];
      ...
      const winPoints = winrateFn(simulatorState.runsMeta, simulatorState.iterations);
      const chartSvg = typeof renderWinrateSvg === "function" ? renderWinrateSvg(winPoints) : "";
